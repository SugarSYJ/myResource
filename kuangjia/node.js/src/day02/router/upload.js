...
module.exports = {
	register: function(app){
		...
		//单位件上传   
		//注意上传界面中的 <input type="file" name="head"/>中的name必须是下面代码中指定的名称  
		// app.post('/upload', uploadSingleHandler, function (req, res, next) {  
		// 	// req.file is the `head` file   
		...
		//多附件上传  
		//注意上传界面中的 <input type="file" name="head"/>中的name必须是下面代码中指定的名  
		app.post('/upload', upload.array('head', 12), function (req, res, next) {  
			// req.files is array of `head` files   
